...
function MatchesTab({
  ...
  const revealWithConfirm = (match: Match) => (side: Side, tallyText: string) => {
    ...
    askConfirm({
      ...
      body:
        match.id === 'F'
          ? `「${name}」 우승으로 공개합니다.\n${tallyText}\n\n스크린에 5-4-3-2-1 카운트다운(6초)이 뜬 뒤 표 카드가 **점점 빠르게** 열리고(패자 표 먼저, 마지막 장이 확정 — 장수와 무관하게 공개 6초) 연출이 끝나면 우승 무대로 자동 전환됩니다 — 클릭 후 약 16초, 표 0건(합의 백업)도 카운트다운 후 우승 무대(카드 연출만 생략, 약 7초). 되돌릴 수 없습니다.`
